# 第二章 数字图像基础
## 1、光和电磁波谱
### 单色光
没有颜色的光称为单色光或者无色光，单色光的唯一属性是其强度
### 灰度级
表示单色光的强度，从黑到白的单色光的度量值范围通常称为灰度级，单色图称为灰度图像
### 彩色光的描述方法
#### 发光强度
从光源流出的能量总量，用瓦特（W）来度量
#### 光通量
观察者从光源感受到的能量，用流明数（lm）度量
#### 亮度
类似于灰度级
### 成像要求
要求“看到”一个物体的电磁波的波长必须小于等于物体的尺寸
## 2、图像取样和量化
多数传感器输出的是连续的电压波形，并不是数字图像，因此有必要把连续的感知数据转化为数字形式，这种话转换包含两种处理：取样和量化。取样和量化才能形成一幅数字图像
### 取样和量化的概念
#### 取样
对坐标值进行数值化称为取样
#### 量化
对幅值数字化称为量化
#### 数字图像质量
取决于取样和量化中所用的样本数和灰度级
### 数字图像表示
#### 表示方法
数字图像------转化为矩阵。矩阵中的每个元素称为图像单元或者像素
数字图像的远点位于左上角，正x轴向下延伸，正y轴向右延伸。这种表示方法基于：矩阵的第一个元素按惯例应在阵列的左上角。
#### 储存空间
M:行数  N：列数  L:离散的灰度级数  
L:灰度级数通常取2的整数次幂，即 L=2^k,这是出于存储硬件的考虑
#### 动态范围
灰度跨越的值域非正式的称为动态范围，是系统中最大可度量灰度与最小可检测灰度之比。上限取决于饱和度，下限取决于噪声。
#### 图像对比度
一幅图像中最高和最低灰度级件的灰度差定义为对比度。当动态范围高时，对比度也高
#### 饱和度
HSV中，饱和度S指的是色彩的鲜艳程度。数字图像处理中，指的是一个灰度值，超过这个灰度值的像素将被填充为这个灰度值
#### 存储大小
存储数字图像所需的比特数b为 b=M*N*k。一幅图像中有2^k个灰度级时，该图像为一幅“k比特图像”。常见的图像均为“8比特图像”，有256个可能的离散灰度值
### 空间和灰度分辨率
#### 空间分辨率
图像中可分辨的最小细节的数量，用单位距离的线对数和单位距离的点数来表示。其中dpi指的是每英寸点数。
#### 灰度分辨率
指灰度级中可分辨的最小变化
### 图像内插
#### 定义
用已知数据来估计未知位置的数值的处理 
#### 分类
最近邻内插、双线性内插、双三次内插（效果最好）  
         $\color{red}{此处插入几种图像内插实现方式的代码}$
## 2.5像素间的基本关系
### 相邻像素
4邻域，8邻域
### 邻接性、连通性、区域和边界
#### 邻接像素
令V是用于定义邻接像素的灰度值集合。在二值图像中，把具有1值得像素归诸于邻接像素，则V={1}。但是在灰度范围0-255中，集合V可包含更多像素，具有V中数值的两个像素即可算邻接
#### 4邻接
#### 8邻接
#### m邻接
1. p,q都在V中
2. q,q为4邻域，或者为8邻域p，q相邻元素不在v中
#### 通路
从具有坐标（x,y）的像素P到具有坐标（s,t）的像素q的通路是特定的像素序列，根据特定的邻接关系，定义为4通路、8通路、m通路
####  $\color{red}{连通集}$
令S是图像中的一个像素子集，如果S的全部像素之间存在一个通路，则可以说两个像素p和q在S中是连通的。  
对于S中的任何像素p，S中连通到该像素的像素集沉稳给S的连通分量。如果S只有一个连通分量，则集合S称为连通集。
####  $\color{red}{区域}$
令R是图像中的一个像素子集，如果R是连通集，则R成为一个区域
####  $\color{red}{邻接区域}$
如果两个区域能联合形成一个连通集，则两个区域称为邻接区域。不邻接的区域称为不连接区域。  
谈论区域时，必须指定邻接类型。
#### 前景
图像中不连接的区域的集合
#### 背景
除去前景后，图像的补给作为背景。
### 距离度量
#### 定义
对于坐标分别为（x,y）,(s,t),(v,w)的像素p,q和z，如果  
（a）D(p,q)>=0    (当且仅当p=q时取等号，非负性)    
（b）D(p,q)=D(q,p)    （可逆性）    
（c）D(p,z)<=D(p,q)+D(q,z)       (三角性)  
 则D时距离函数或者度量
 #### 分类
 1. 欧式距离：平方距离，距中心点相同距离的点是一个圆（二维）
 2. D4距离：城市街区距离，距中心点相同距离的点是一个菱形（二维）
 3. D8距离：方形距离
## 数学工具
### 阵列操作和矩阵操作
阵列相乘和矩阵相乘  
默认都是使用阵列操作作为图像之间的操作
### 线性操作和非线性操作
线性算子：加减一幅图像或者乘除常数  
非线性算子：最大值，最小值，relu等
### 算术操作
相加平均：降噪，通过相加平均，使噪声图像的期望等于原图
图像相减：差别增强，增强后会对像素的灰度级进行重定义
图像相乘和相除：  
1. 消除阴影:一个成像传感器产生的图像g（x,y）=f(x,y)*h(x,y),其中f是完美图像，h是阴影函数。通过g/h是一个得到近似的完美图像。h可以通过采集一幅具有恒定灰度的目标来得到
2. 感兴趣区域操作（ROI）：模板图像的ROI区域为1，其余地区为0，相乘就能留下感兴趣区域。
### 像素重新定义到0-255的方法（标定）
提取最小值，并把它的负值加到所有差值图像中，通过255/max去乘以每个像素值
### 集合和逻辑操作
灰度值的并集与交集操作通常是分别定义为相应像素最大和最小值  
补集操作定义为常数与每个图像中每个像素的灰度间的两两之差
### 模糊集合
不适用固定的确切的阈值
### 空间操作
#### 单像素操作
以灰度值为基础，改变单个像素的值
#### 邻域操作
邻域算子进行平滑等操作
#### 几何空间变换
几何变换由两个基本操作组成：（1）坐标的空间变换；（2）灰度值内插，对空间变换后的像素赋灰度值
#### 仿射变换
一般形式为：  

![avatar](../src/2-1.png)  
仿射变换可根据矩阵T中元素所选择的值，对一组坐标点做尺度、旋转、平移或者偏移。
![avatar](../src/2-2.png)  
以上操作将图像的像素重新定位到已给新的位置，为了完成仿射变换，还需要对新位置赋予灰度值，这可用灰度内插的方法完成。
#### 仿射变换实现的两种形式
1. 前向映射：使用仿射变换的一般形式对图像进行变换
2. 反向映射：扫描输出像素的位置，并在每个位置使用（v,w）=T-1(x,y)计算输入图像中的相应位置，内插使用进嘴的输入像素之一决定输入像素的灰度值。这种方法更有效。
#### 图像配准
图像配准用于对齐两幅或者多福相同场景的图像。  
假如输入图像和参考图像中有一组约束点（x,y）和（v，u）  
x=c1*v+c2*w+c3*v*w+c4  
y=c5*v+c6*w+c7*v*w+c8
如果两幅图像中有4组相应的约束点，那么就能写8个方程，解出8个未知数。
###  图像变换
 $\color{red}{牵涉到频率域，暂时不讨论}$
 ### 概率方法
  $\color{red}{暂时不讨论}$

